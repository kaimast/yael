...
TcpSocket::TcpSocket(MessageMode mode, size_t max_send_queue_size)
    : m_port(0), m_is_ipv6(false), m_fd(-1),
      m_max_send_queue_size(max_send_queue_size) {
...
TcpSocket::~TcpSocket() { TcpSocket::close(true); }

bool TcpSocket::has_messages() const { return m_slicer->has_messages(); }
...
bool TcpSocket::create_fd() {
    ...
    ::setsockopt(m_fd, IPPROTO_TCP, TCP_NODELAY,
                 reinterpret_cast<const char *>(&TRUE_FLAG), sizeof(TRUE_FLAG));
...
bool TcpSocket::bind_socket(const Address &address) {
    ...
    if (m_is_ipv6) {
        ...
        if (::bind(m_fd, reinterpret_cast<sockaddr *>(&sock_addr),
                   sizeof(sock_addr)) != 0) {
            return false;
        }
    } else {
...
bool TcpSocket::connect(const Address &address, const std::string &name) {
    ...
    if (m_is_ipv6) {
        ...
        if (::connect(m_fd, reinterpret_cast<const sockaddr *>(&sock_addr),
                      sizeof(sock_addr)) != 0) {
            close();
            return false;
        }
...
std::vector<std::unique_ptr<Socket>> TcpSocket::accept() {
    ...
    while (true) {
        ...
        if (fd >= 0) {
            auto ptr =
                new TcpSocket(m_slicer->type(), fd, m_max_send_queue_size);
...
void TcpSocket::calculate_remote_address() {
    ...
    inet_ntop(AF_INET, dynamic_cast<in_addr *>(&sin.sin_addr), &ipstring[0],
              16);
...
bool TcpSocket::receive_data(buffer_t &buffer) {
    ...
    if (buffer.is_valid()) {
        throw std::runtime_error("TcpSocket::receive_data failed: Still have "
                                 "data queued up in buffer");
    }
...
bool TcpSocket::send(std::shared_ptr<uint8_t[]> &data, uint32_t len,
                     bool async) {
    if (len <= 0) {
        throw socket_error("Message size has to be > 0");
    }
...
bool TcpSocket::do_send() {
    ...
    while (true) {
        ...
        // this way other threads can queue up messages while we write to the
        // socket
        ...
        while (message.sent_pos < length) {
            auto s = ::write(m_fd, rdata + message.sent_pos,
                             length - message.sent_pos);

            if (s > 0) {
                message.sent_pos += s;
            } else if (s == 0) {
                LOG(WARNING)
                    << "Connection lost during send: Message may only be sent "
                       "partially";
                close(true);
                return false;
            } else if (s < 0) {
