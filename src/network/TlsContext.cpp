...
void TlsContext::tls_verify_cert_chain(
...
void TlsContext::tls_emit_data(const uint8_t data[], size_t size) {
    ...
    while (sent < size) {
        auto s =
            ::write(m_socket.get_fd(),
                    reinterpret_cast<const char *>(data) + sent, size - sent);

        if (s > 0) {
            sent += s;
        } else if (s == 0) {
            LOG(WARNING) << "Connection lost during send: Message may only be "
                            "sent partially";
            m_socket.close();
            return;
        } else if (s < 0) {
...
void TlsContext::close() { m_channel->close(); }

void TlsContext::tls_record_received(uint64_t seq_no, const uint8_t data[],
                                     size_t size) {
    (void)seq_no;
    ...
    while (pos < size) {
        ...
        auto cpy_size =
            std::min<size_t>(yael::network::buffer_t::MAX_SIZE, size - pos);
...
TlsServer::TlsServer(TlsSocket &socket, const std::string &key_path,
...
TlsClient::TlsClient(TlsSocket &socket) : TlsContext(socket) {
