...
Acceptor::Acceptor(uint16_t port) {
    ...
    yael::NetworkSocketListener::set_socket(
        std::unique_ptr<network::Socket>(socket), yael::SocketType::Acceptor);
    LOG(INFO) << "Listening for peers on host " << host << " port " << port;
}

Peer::Peer(const std::string &host, uint16_t port, uint32_t delay)
    : yael::DelayedNetworkSocketListener(delay, nullptr,
...
Peer::Peer(std::unique_ptr<yael::network::Socket> &&s, uint32_t delay)
    : yael::DelayedNetworkSocketListener(
          delay, std::forward<std::unique_ptr<yael::network::Socket>>(s),
          yael::SocketType::Connection) {}
...
void do_connect(int argc, char **argv) {
    ...
    for (size_t i = 0; i < num_children; ++i) {
        ...
        std::cout << "[" << i + 1 << "/" << num_children << "] Child with PID "
                  << (long)pid << " exited with status 0x" << status << "."
                  << std::endl;
    }
...
void print_help() {
    std::cout << "usage: " << std::endl
              ...
              << "   ./multi-client-test connect <upstream-host> "
                 "<upstream-port> <num_connection> <delay>"
              << std::endl
              << std::endl;
    exit(1);
}
